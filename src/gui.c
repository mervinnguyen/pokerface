...
{
	// GTK Variables
	// GtkWidget *Client_WINDOW = NULL;				// Main window
	...
	// // 2nd window test
	// // Create a new window
	// window_client_game = gtk_window_new(GTK_WINDOW_TOPLEVEL);
	// // Update the settings for the window
	// gtk_window_set_title(GTK_WINDOW(window_client_game), "Game");
	// gtk_window_set_default_size(GTK_WINDOW(window_client_game), WINDOW_WIDTH, WINDOW_HEIGHT);
	// // Center GTK window
	// gtk_window_set_position(GTK_WINDOW(window_client_game), GTK_WIN_POS_CENTER);
	// gtk_container_set_border_width(GTK_CONTAINER(window_client_game), 10);
	// // Add poker icon to window
	// pokerIcon = gdk_pixbuf_new_from_file("gui_images/PokerIcon.png", NULL); 
  	// gtk_window_set_icon(GTK_WINDOW(window_client_game), pokerIcon);
	...
	// Move variable declaration to top later
	// Create the first page
	GtkWidget *client_page_1 = gtk_event_box_new();
	gtk_notebook_append_page(GTK_NOTEBOOK(Client_NOTEBOOK), client_page_1, NULL);
	// Change Notebook Color
    GdkColor color_1;
    gdk_color_parse("#f5f5f5", &color_1); // Parse color in hexadecimal format
    gtk_widget_modify_bg(client_page_1, GTK_STATE_NORMAL, &color_1);
	

	// Create a vertical box for alignment
	CMM_vertical_ALIGNMENT = gtk_vbox_new(FALSE, 10);
	gtk_container_add(GTK_CONTAINER(client_page_1), CMM_vertical_ALIGNMENT);
